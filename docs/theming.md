...
- Brand tokens (colors) sourced from green-tech-hub.com's real palette (`#1FBE1E` primary — see `theme/tokens.py`) and reused everywhere, rather than each service picking its own accent color. The logo (`static/logo/logo.png`, vendored from `GreenMachine582/GreenTechHub` — see `static/VENDORED.md` for the source commit SHA) is the source of truth for both the navbar logo and the favicon, since GreenTechHub itself reuses the same file for both; there's no separate dark-mode or favicon-only variant to vendor. `theme.brand_context(show_logo=True, static_url_prefix=...)` opts a consumer into it (`logo_url`/`favicon_url` both `None` by default, so adopting this doesn't change existing consumers' output). Re-vendor on change and record the new commit SHA in `static/VENDORED.md`.
